...
/**
 * npm's square mark, for the download count above the landing page's headline.
 *
 * Drawn in its own brand colour, `#cb3837`, fixed in both themes, for the
 * reason Figma's and React's are: the red square is most of what makes the mark
 * npm's at 16px, and in the page's ink it is a grey tile with a notch. The path
 * is the published one from npm's entry in Simple Icons, not redrawn.
 */
export function NpmLogo(props: LogoProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
...
/**
 * The first of the three framework marks on the landing page's install row.
 * Vue's and Svelte's are below it.
 *
 ...
 *
 * **These are the two that were drawn by eye and deleted.** The note on
 * `ReactLogo` promised that when a second framework genuinely shipped they
 * would come back as the projects' own published paths rather than as
 * reconstructions, and this is that: both are simple-icons' current marks,
 * dropped in unchanged at their own 24-unit box, the same provenance as the
 * four social marks below. Neither has been re-solved against this set's grid,
 * which is the point.
 *
 * In their official colours and fixed in both themes, like React's above. A
 * framework chip is read at 16px, where colour is most of what identifies a
 * mark, and a trademark is not part of this site's palette. `#4FC08D` and
 * `#FF3E00` are simple-icons' recorded values, not eyeballed ones.
 *
 * Solid single paths carrying no stroke, so they take no `strokeWidth` of their
 * own and cannot be mistaken for glyphs from the set drawn at the wrong weight.
 */
export function VueLogo(props: LogoProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
...
export function SvelteLogo(props: LogoProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
